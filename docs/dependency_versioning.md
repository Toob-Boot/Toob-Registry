...
| Feld             | Wert                                             | Quelle              |
| ---------------- | ------------------------------------------------ | ------------------- |
| **Pfad**         | `drivers/{category}/{name}/driver_manifest.json` | Dateisystem         |
| **Version**      | `1.0.1`                                          | Manuell im Manifest |
| **Abhängig von** | Nichts                                           | —                   |
| **Vererbt an**   | Chip (über `sources.drivers` Array)              | —                   |

```json
// drivers/clock/esp_systimer/driver_manifest.json
{ "name": "esp_systimer", "version": "1.0.1", "description": "..." }
```

**Code-Dateien:** Spezifische Treiber-Dateien wie `.c` und `.h` Dateien für exakt diesen Treiber.
...
// integrations/zephyr/integration_manifest.json
{
  "name": "zephyr",
  "version": "1.0.0",
  "description": "Zephyr RTOS Integration"
}
```
...
| **Abhängig von** | `arch`, `drivers`, `toolchain`    | Über Felder `arch`, `sources.drivers`, `compiler_prefix` |
...
// chips/esp32c6/chip_manifest.json
{
  ...
  "min_compiler": "latest",
  "sources": {
    "drivers": ["drivers/clock/esp_systimer", "drivers/rtc/esp_rtc_mem"]
  }
}
```
...
**Die `environment_hash` Logik:** SHA-256 über `chip + toolchain + arch` JSON.
Ändert sich irgendein Byte in einer der Quellen oder ändert sich die Chip-Version (z.B. durch geerbten Treiber-Bump), ändert sich der Hash → alle alten Matrix-Tests werden invalidiert.
...
```
Commit: "fix(drivers/rtc): update memory retention"
  → drivers/rtc/esp_rtc_mem geändert
...
Zusammengeführt in einem PR-Merge:
  ...
  1. drivers/rtc/esp_rtc_mem: 1.0.1 → 1.0.2 (PATCH)
  2. arch/riscv32:            1.0.0 → 1.1.0 (MINOR)
  3. esp32c6:                 1.1.0 → 1.2.0 (MINOR, weil arch=MINOR > driver=PATCH)
...
| **Basis-Dependencies (Drivers/Arch/Toolchain) werden nie auto-gebumped** | —                               | Version muss immer manuell in der Manifest-Datei eingetragen werden. Kein Script erkennt Dateiänderungen automatisch.                    |
...
| Wer bumped Drivers/Arch/Toolchain? | Entwickler manuell                      | Automatisch durch Commit-Message-Konvention ODER Version-Diff    |
...
| Schritt              | IST                                     | SOLL                                                  |
| -------------------- | --------------------------------------- | ----------------------------------------------------- |
| Driver ändert sich   | Manuell Version bumpen                  | Manuell ODER automatisch via `git diff`               |
...
| Szenario                     | IST                          | SOLL                                 |
| ---------------------------- | ---------------------------- | ------------------------------------ |
| Driver PATCH + Arch MINOR    | Chip = MINOR (hardcoded)     | Chip = MINOR (max(PATCH, MINOR))     |
| Driver MAJOR                 | Chip = MINOR (hardcoded)     | Chip = MAJOR (vererbt)               |
...
│        - drivers/*/* geändert? → Lese alte + neue       │
│          driver_manifest.json Version → berechne Typ    │
...
WICHTIG: Obwohl wir nur für die gesamte Registry offizielle Git-Tags (wie `v1.0.7`) anlegen, besitzt jede einzelne interne Komponente (Driver, Arch, Toolchain, Chip) ihre eigene, unabhängige Versionierung!

**Wie funktioniert das ohne Git-Tags für Drivers/Chips?**

- Jede Komponente hat eine eigene `*_manifest.json` (z.B. `driver_manifest.json`), in der ein `"version"` Feld steht (z.B. `"1.0.1"`).
- Diese Manifeste _sind_ die Source of Truth für die einzelnen Schichten.
- Wenn die Pipeline prüft, ob sich z.B. der `esp_systimer` Driver geändert hat, sucht sie nicht nach einem Git-Tag namens `esp_systimer-v1.0.1`. Stattdessen führt sie `git show BEFORE_SHA:drivers/clock/esp_systimer/driver_manifest.json` aus. Sie zieht sich also den exakten Dateiinhalt des Manifests, wie er im vorherigen Commit aussah.
